...
FName GetDemoName()
{
	...
	return Ver.IsNone() || Ver.ToString() == TEXT(" ") || Ver.ToString().TrimStartAndEnd().IsEmpty() ? NAME_None : Ver;
}
	...
	if (Owner && Class && VerifySaveTag(Tag))
	{
		...
		if (UToroSaveObject* NewObj = NewObject<UToroSaveObject>(Owner, Class, *Name))
		{
			...
			NewObj->SavePath = FString::Printf(TEXT("%sSaveGames/%s/%s.tsave"), *FPaths::ProjectSavedDir(),
				*GetDemoName().ToString(), *Name).Replace(TEXT("None/"), TEXT(""));
			
			NewObj->OnCreation();
			NewObj->LoadObject(nullptr);
			UE_LOG(LogTemp, Warning, TEXT("Created Save Object For: %s"), *NewObj->SavePath);
			return NewObj;
		}
	}
