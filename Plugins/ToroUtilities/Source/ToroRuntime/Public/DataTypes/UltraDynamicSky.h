...
class TORORUNTIME_API UWeatherDataAsset final : public UPrimaryDataAsset
{
	GENERATED_BODY()
};
...
class TORORUNTIME_API IUDSInterface
{
	GENERATED_BODY()
...
public:
	...
	static void SetCustomSettings(AActor* Target, const FUDSSettings& InSettings)
	{
		if (IsValid(Target) && Target->Implements<UUDSInterface>())
		{
			Execute_SetCustomSettings(Target, InSettings);
		}
	}
};