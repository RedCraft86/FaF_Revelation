...
void FWTaskActorVisibility::RunEvent(const UObject* WorldContext)
{
	FOR_EACH_SOFT_PTR(Targets, AActor, {
		Ptr->SetActorHiddenInGame(ChangeType == EWorldActionType::Set ? !bVisible : !Ptr->IsHidden());
	})
}
...
{
	FOR_EACH_SOFT_PTR(Targets, AActor, {
		Ptr->SetActorEnableCollision(ChangeType == EWorldActionType::Set ? bCollision : !Ptr->GetActorEnableCollision());
	})
}
...
void FWTaskActorEnabled::RunEvent(const UObject* WorldContext)
{
	FOR_EACH_SOFT_PTR(Targets, AActor, {
		...
		{
			ToroActor->SetEnabled(ChangeType == EWorldActionType::Set
				? bEnabled : !ToroActor->IsEnabled());
		}
		else if (AToroVolume* ToroVolume = Cast<AToroVolume>(Ptr))
		{
			ToroVolume->SetEnabled(ChangeType == EWorldActionType::Set
