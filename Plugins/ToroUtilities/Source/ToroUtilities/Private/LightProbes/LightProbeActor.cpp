...
ALightProbeActor::ALightProbeActor(): Intensity(1.0f), Radius(500.0f), Falloff(2.0f), Color(FLinearColor::White)
	, bDisableWithLumen(false), MaxDistance(3000.0f), FadeRange(100.0f)
{
	...
	WireShape = CreateEditorOnlyDefaultSubobject<UWireShapeComponent>("WireShape");
...
void ALightProbeActor::OnConstruction(const FTransform& Transform)
{
	...
	if (WireShape)
	{
		...
		WireShape->WireSpheres.Add(TEXT("Radius"), Data);
	}
	if (IconBillboard)
	{
		if (!IconMaterial)
