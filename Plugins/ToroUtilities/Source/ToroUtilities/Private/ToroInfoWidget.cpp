...
void UToroInfoWidget::UpdateFrameRate() const
{
...
void UToroInfoWidget::OnGameFocusChanged(const bool bFocused) const
{
	UnfocusedView->SetVisibility(bFocused ? ESlateVisibility::Collapsed : ESlateVisibility::HitTestInvisible);
}

void UToroInfoWidget::OnSettingsChanged(const UToroUserSettings* Settings)
{
...
void UToroInfoWidget::OnSaveLoad(const UToroSaveObject* SaveObject, const ESaveGameActivity Activity)
{
...
void UToroInfoWidget::InitWidget()
{
	Super::InitWidget();
	UnfocusedView->SetVisibility(ESlateVisibility::Collapsed);
	if (UToroUserSettings* Settings = UToroUserSettings::Get())
	{
		Settings->OnSettingsApplied.AddUObject(this, &UToroInfoWidget::OnSettingsChanged);
		Settings->OnDynamicSettingsChanged.AddUObject(this, &UToroInfoWidget::OnSettingsChanged);
		OnSettingsChanged(Settings);
	}

	if (AToroPlayerController* PC = AToroPlayerController::Get(this))
	{
		PC->OnGameFocusChanged.AddUObject(this, &UToroInfoWidget::OnGameFocusChanged);
	}

	if (UToroSaveManager* SaveManager = UToroSaveManager::Get(this))
	{
		SaveManager->OnSaveIO.AddUObject(this, &UToroInfoWidget::OnSaveLoad);
	}
}

void UToroInfoWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
