...
AMasterPostProcess* AMasterPostProcess::Get(const UObject* ContextObject, const bool bCreateIfNotFound)
{
	...
	{
		UE_LOG(LogToroUtilities, Warning, TEXT("Multiple Master Post Process actors found! Returning the first one."));
...
{
	if (!UToroSettings::Get()->IsUsingLightProbes()) return;
...
void AMasterPostProcess::BeginPlay()
{
	...
	if (PostProcesses.Num() > 1)
	{
		UE_LOG(LogToroUtilities, Warning, TEXT("Multiple Master Post Process actors found!"));
