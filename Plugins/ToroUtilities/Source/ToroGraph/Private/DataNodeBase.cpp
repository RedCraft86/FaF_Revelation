...
UDataNodeBase::UDataNodeBase(): NodeID(FGuid::NewGuid()), BackgroundColor(0.1f, 0.1f, 0.1f)
	, ContextMenuName(NSLOCTEXT("ToroUtilities", "DataNodeName", "Generic Data Node"))
	, ParentLimit(255), ChildLimit(255), CompatibleGraph(UDataGraphBase::StaticClass())
{}
...
{
	return Name.IsEmptyOrWhitespace() ? INVTEXT("Unnamed Data Node") : Name;
}
...
bool UDataNodeBase::CanConnectFrom(UDataNodeBase* Other, uint8 NumParentNodes, FString* ErrorMsg)
{
	if (!Other)
	{
...
bool UDataNodeBase::CanConnectTo(UDataNodeBase* Other, uint8 NumChildNodes, FString* ErrorMsg)
{
	if (!Other)
	{
