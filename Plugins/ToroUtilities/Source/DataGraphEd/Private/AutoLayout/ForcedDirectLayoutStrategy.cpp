...
FBox2D UForcedDirectLayoutStrategy::LayoutOneTree(UDataNodeBase* RootNode, const FBox2D& PreTreeBound)
{
	...
	for (int32 IterationNum = 0; IterationNum < MaxIteration; ++IterationNum)
	{
		...
		while (!CurrLevelNodes.IsEmpty())
		{
			for (int32 i = 0; i < CurrLevelNodes.Num(); ++i)
			{
				...
				UDataGraphEdNode* EdNode_ParentNode = EdGraph->NodeMap[Node];

				for (int32 j = 0; j < Node->ChildNodes.Num(); ++j)
				{
					NextLevelNodes.Add(Node->ChildNodes[j]);

					UDataGraphEdNode* EdNode_ChildNode = EdGraph->NodeMap[Node->ChildNodes[j]];
