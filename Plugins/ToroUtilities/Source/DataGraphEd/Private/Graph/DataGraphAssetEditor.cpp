...
void FDataGraphAssetEditor::InitEditor(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, UDataGraphBase* Graph)
{
	EditingGraph = Graph;
	CreateEdGraph();
...
void FDataGraphAssetEditor::CreateEdGraph() const
{
	...
	{
		EditingGraph->EdGraph = CastChecked<UDataGraphEdGraph>(FBlueprintEditorUtils::CreateNewGraph(EditingGraph,
			NAME_None, UDataGraphEdGraph::StaticClass(), UDataGraphEdSchema::StaticClass()));
...
void FDataGraphAssetEditor::RebuildDataGraph() const
{
	...
	UDataGraphEdGraph* EdGraph = Cast<UDataGraphEdGraph>(EditingGraph->EdGraph);
	check(EdGraph);
...
void FDataGraphAssetEditor::DeleteSelectedNodes() const
{
	...
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		...
		if (UDataGraphEdNode* EdNode_Node = Cast<UDataGraphEdNode>(EdNode))
		{
			EdNode_Node->Modify();
...
void FDataGraphAssetEditor::CopySelectedNodes() const
{
	...
	for (FGraphPanelSelectionSet::TIterator SelectedIter(SelectedNodes); SelectedIter; ++SelectedIter)
	{
		...
		if (UDataGraphEdEdge* EdNode_Edge = Cast<UDataGraphEdEdge>(*SelectedIter))
		{
			const UDataGraphEdNode* StartNode = EdNode_Edge->GetStartNode();
			const UDataGraphEdNode* EndNode = EdNode_Edge->GetEndNode();
...
void FDataGraphAssetEditor::PasteNodesHere(const FVector2f& Location) const
{
	const TSharedPtr<SGraphEditor> CurrentGraphEditor = GetCurrGraphEditor();
...
void FDataGraphAssetEditor::AutoArrange() const
{
	...
	switch (UDataGraphEdSettings::Get()->AutoLayoutStrategy)
	{
...
{
	return EditingGraph && Cast<UDataGraphEdGraph>(EditingGraph->EdGraph);
}
...
{
	const UDataGraphEdGraph* EdGraph = Cast<UDataGraphEdGraph>(EditingGraph->EdGraph);
	check(EdGraph);
