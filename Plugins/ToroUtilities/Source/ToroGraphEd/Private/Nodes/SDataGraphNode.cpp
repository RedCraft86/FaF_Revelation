...
public:
	...
	void Construct(const FArguments& InArgs, UEdGraphPin* InPin)
	{
		...
		SBorder::Construct(SBorder::FArguments()
			.BorderBackgroundColor(ToroGraphColors::DefaultPin)
...
protected:
	...
	virtual TSharedRef<FDragDropOperation> SpawnPinDragEvent(const TSharedRef<SGraphPanel>& InGraphPanel, const TArray<TSharedRef<SGraphPin>>& InStartingPins) override
	{
		...
		return FDataGraphDragDrop::New(InGraphPanel, PinHandles);
	}

};

void SDataGraphNode::Construct(const FArguments& InArgs, UEdDataGraphNode* InNode)
{
	GraphNode = InNode;
	InNode->SNode = this;
	UpdateGraphNode();
...
{
	const UEdDataGraphNode* Node = CastChecked<UEdDataGraphNode>(GraphNode);
...
{
	const UEdDataGraphNode* Node = CastChecked<UEdDataGraphNode>(GraphNode);
	return Node ? Node->GetTextID() : INVTEXT("Unknown ID");
}

FSlateColor SDataGraphNode::GetBackgroundColor() const
{
	const UEdDataGraphNode* Node = CastChecked<UEdDataGraphNode>(GraphNode);
	return Node ? Node->GetBackgroundColor() : ToroGraphColors::InvalidBackground;
}

void SDataGraphNode::OnNameCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	OnNameTextCommited(InText, CommitInfo);
	UEdDataGraphNode* Node = CastChecked<UEdDataGraphNode>(GraphNode);
...
{
	UEdDataGraphNode* StateNode = CastChecked<UEdDataGraphNode>(GraphNode);
