...
#include "Nodes/DataGraphDragDrop.h"
#include "Nodes/EdDataGraphNode.h"
#include "SGraphPanel.h"

TSharedRef<FDataGraphDragDrop> FDataGraphDragDrop::New(const TSharedRef<SGraphPanel>& InGraphPanel, const TArray<FGraphPinHandle>& InStartingPins)
{
	TSharedRef<FDataGraphDragDrop> Operation = MakeShareable(new FDataGraphDragDrop(InGraphPanel, InStartingPins));
	Operation->Construct();
	return Operation;
}

void FDataGraphDragDrop::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
	GraphPanel->OnStopMakingConnection();
	FGraphEditorDragDropAction::OnDrop(bDropWasHandled, MouseEvent);
}

void FDataGraphDragDrop::HoverTargetChanged()
{
	...
	else if (const UEdDataGraphNode* TargetNodeObj = Cast<UEdDataGraphNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(ValidSourcePins);
		for (const UEdGraphPin* StartingPinObj : ValidSourcePins)
		{
			...
			else
			{
				Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW,
					NSLOCTEXT("ToroGraphEd", "PinError", "Not a valid UEdDataGraphNode"));
			}
...
FReply FDataGraphDragDrop::DroppedOnPin(const FVector2f& ScreenPosition, const FVector2f& GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(ValidSourcePins);
...
FReply FDataGraphDragDrop::DroppedOnNode(const FVector2f& ScreenPosition, const FVector2f& GraphPosition)
{
...
FReply FDataGraphDragDrop::DroppedOnPanel(const TSharedRef<SWidget>& Panel, const FVector2f& ScreenPosition, const FVector2f& GraphPosition, UEdGraph& Graph)
{
...
void FDataGraphDragDrop::OnDragged(const FDragDropEvent& DragDropEvent)
{
...
void FDataGraphDragDrop::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
...
FDataGraphDragDrop::FDataGraphDragDrop(const TSharedRef<SGraphPanel>& InGraphPanel, const TArray<FGraphPinHandle>& DraggedPins)
