...
void SDataGraphEdge::Construct(const FArguments& InArgs, UEdDataGraphEdge* InNode)
{
	GraphNode = InNode;
	UpdateGraphNode();
}
...
void SDataGraphEdge::OnNameCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	...
	UEdDataGraphEdge* MyNode = CastChecked<UEdDataGraphEdge>(GraphNode);
...
void SDataGraphEdge::PerformSecondPassLayout(const TMap<UObject*, TSharedRef<SNode>>& NodeToWidgetLookup) const
{
	...
	UEdDataGraphEdge* EdgeNode = CastChecked<UEdDataGraphEdge>(GraphNode);
...
{
	const UEdDataGraphEdge* Node = CastChecked<UEdDataGraphEdge>(GraphNode);
