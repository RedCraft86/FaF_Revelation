...
class FDataGraphToolbar;

class FDataGraphAssetEd final : public FAssetEditorToolkit, public FNotifyHook, public FGCObject
{
public:
	
	FDataGraphAssetEd();
	virtual ~FDataGraphAssetEd() override;

	void InitEditor(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, UDataGraphBase* Graph);
	...
	// FGCObject interface
	virtual FString GetReferencerName() const { return TEXT("FAssetEditor_LTDataGraphBase"); }
	// ~FGCObject interface

	void RegisterToolbarTab(const TSharedRef<FTabManager>& InTabManager);
	TSharedPtr<FDataGraphToolbar> GetToolbarBuilder() { return ToolbarBuilder; }
...
private:
	...
	TObjectPtr<UDataGraphBase> EditingGraph;
	TSharedPtr<FDataGraphToolbar> ToolbarBuilder;
