...
AStaticMeshActor* FStaticMeshBaker::SpawnActor(class UEditorActorSubsystem* Subsystem, const FStaticMeshData& MeshData,
	const FTransform& Transform, const FString& Label, const FString& Folder)
{
	if (!Subsystem || !MeshData.IsValidData()) return nullptr;
		...
		Transform.GetTranslation(), Transform.GetRotation().Rotator())))
	{
		...
		MeshData.ToStaticMeshComponent(NewActor->GetStaticMeshComponent());
