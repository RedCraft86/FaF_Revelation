...
private:
	...
	virtual void DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI) override
	{
		...
		{
			if (FMath::IsNearlyZero(ConeComponent->OuterAngle) || FMath::IsNearlyEqual(ConeComponent->GetInnerAngle(), 90.0f))
			{
	...
	virtual void DrawVisualizationHUD(const UActorComponent* Component, const FViewport* Viewport, const FSceneView* View, FCanvas* Canvas) override
	{
		if (const UVisionConeComponent* ConeComponent = Cast<const UVisionConeComponent>(Component))
		{
			VisualizerHelpers::DrawText(View, Canvas, ConeComponent->GetComponentLocation(), FText::Format(INVTEXT("Trace: {0}"), 
				ConeComponent->TraceMethod ? ConeComponent->TraceMethod->GetClass()->GetDisplayNameText() : INVTEXT("None")),
