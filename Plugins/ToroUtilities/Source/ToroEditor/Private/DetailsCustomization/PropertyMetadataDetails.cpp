...
#define MAKE_TOOLTIP(Key, Text) FSlateApplicationBase::Get().MakeToolTip(LOCTEXT(Key, Text))

TSharedPtr<IDetailCustomization> FPropertyMetadataCustomization::MakeInstance(TSharedPtr<IBlueprintEditor> InBlueprintEditor)
{
	...
	if (Objects && Objects->Num() == 1)
	{
		...
		{
			return MakeShared<FPropertyMetadataCustomization>(InBlueprintEditor, Blueprint);
		}
	}
...
void FPropertyMetadataCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	auto ToggleMetadata = [this, PropertyName](const ECheckBoxState State, const FName MetadataKey)
	{
	...
	auto OnRemoveMetadata = [this, PropertyName](const FName MetadataKey)
	{
		RemoveMetadata(PropertyName, MetadataKey);
	};

	auto OnMetaKeyChanged = [this, PropertyName](const FText& NewKey, ETextCommit::Type ChangeType, const FName OldKey)
	{
		ChangeMetadataKey(PropertyName, OldKey, FName(*NewKey.ToString()));
	};

	auto OnMetaValueChanged = [this, PropertyName](const FText& NewValue, ETextCommit::Type ChangeType, const FName MetadataKey)
	{
	...
	IDetailCategoryBuilder& VariableGroup = DetailLayout.EditCategory("Variable");

	IDetailGroup& MetadataGroup = VariableGroup.AddGroup("MetadataGroup", LOCTEXT("MetadataGroupName", "Metadata"));
	...
	if (Property->IsA(FStructProperty::StaticClass()))
	{
		...
		[
			SNew(STextBlock)
			.ToolTip(MAKE_TOOLTIP("StructInnerTooltip", "Inner properties of structs will not be placed in a separate group."))
	...
	{
		RemoveMetadata(PropertyName, "ForceInlineRow");
	}
...
TArray<FBPVariableMetaDataEntry> FPropertyMetadataCustomization::GetMetadataEntries(const int PropertyIndex) const
{
...
FSlateFontInfo FPropertyMetadataCustomization::GetFont(const int Size, const bool bBold)
{
...
FProperty* FPropertyMetadataCustomization::GetInnermostProperty(const TWeakFieldPtr<FProperty>& Property)
{
	// Hopefully this never nulls since the editor will crash lol
...
bool FPropertyMetadataCustomization::IsPropertyOfType(const TWeakFieldPtr<FProperty>& Property, const FFieldClass* FieldType)
{
	if (!Property.IsValid()) return false;

	// Maps are a special case since the metadata seems to apply to both the Key and the Value
...
bool FPropertyMetadataCustomization::IsPropertyOfStruct(const TWeakFieldPtr<FProperty>& Property, const UScriptStruct* Struct)
{
	if (!Property.IsValid()) return false;

	// For some reason metadata adjustments for structs in container type properties don't apply
...
bool FPropertyMetadataCustomization::IsMapWithStructKey(const TWeakFieldPtr<FProperty>& Property)
{
	if (!Property.IsValid()) return false;
	if (const FMapProperty* MapProp = CastField<FMapProperty>(Property.Get()))
	{
...
int FPropertyMetadataCustomization::GetMetadataCount(const FName Property) const
{
...
bool FPropertyMetadataCustomization::DoesMetadataExist(const FName Property, const FName Key) const
{
...
void FPropertyMetadataCustomization::RemoveMetadata(const FName Property, const FName Key) const
{
...
FString FPropertyMetadataCustomization::GetMetadataValue(const FName Property, const FName Key) const
{
...
void FPropertyMetadataCustomization::SetMetadataValue(const FName Property, const FName Key, const FString& Value) const
{
...
void FPropertyMetadataCustomization::ChangeMetadataKey(const FName Property, const FName OldKey, const FName NewKey) const
{
...
void FPropertyMetadataCustomization::ChangeMetadataValue(const FName Property, const FName Key, const FString& Value, bool bDeleteEmpty) const
{
	if (Property.IsNone() || Key.IsNone()) return;
...
void FPropertyMetadataCustomization::VerifyMetadata(const FName Property, const FName Key) const
{
	if (Property.IsNone() || Key.IsNone()) return;
