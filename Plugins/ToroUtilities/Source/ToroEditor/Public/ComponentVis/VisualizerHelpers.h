...
{
	TOROEDITOR_API inline void DrawArcWithThickness(FPrimitiveDrawInterface* PDI, const FVector& Base, const FVector& X,
		...
		const bool bDrawShadow = false, const UFont* Font = nullptr)
	{
			...
			View->ViewMatrices.GetViewProjectionMatrix(), ScreenPos))
		{
			...
			FCanvasTextItem TextItem = FCanvasTextItem(ScreenPos, Text, {Font ? Font
				: GEngine->GetSmallFont(), FontScale > 0 ? (float)FontScale : 12.0f, NAME_None}, Color);
