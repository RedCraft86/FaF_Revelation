...
#define LOG_THIS_LINE(Category, Verbosity) UE_LOG(Category, Verbosity, TEXT("[File: %hs] [Line: %d]"), __FILE__, __LINE__);
#define UE_PRINT(Time, Color, Format, ...) GEngine->AddOnScreenDebugMessage(-1, Time, FColor::Color, FString::Printf(Format, __VA_ARGS__));
#define UE_LOG_KISMET(Verbosity, Format, ...) FFrame::KismetExecutionMessage(*FString::Printf(Format, __VA_ARGS__), ELogVerbosity::Verbosity);
