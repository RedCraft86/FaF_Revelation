...
private:
		...
		IPropertyTypeCustomizationUtils& StructCustomizationUtils) override
	{
		IDetailGroup& Footsteps = StructBuilder.AddGroup(TEXT("Footsteps"), StructPropertyHandle->GetPropertyDisplayName());
		{
			...
			Footsteps.AddPropertyRow(STRUCT_PROPERTY(Walk));
			Footsteps.AddPropertyRow(STRUCT_PROPERTY(Run));
			Footsteps.AddPropertyRow(STRUCT_PROPERTY(Crouch));
		}
	}
};
#undef STRUCTNAME