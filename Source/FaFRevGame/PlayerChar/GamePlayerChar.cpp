...
AGamePlayerChar::AGamePlayerChar()
{
	...
	Inspection = CreateDefaultSubobject<UInspectionComponent>("Inspection");
	Inspection->SetRelativeLocation(FVector(20.0f, -9.0f, 0.0f));
	Inspection->SetupAttachment(PlayerCamera);
#if WITH_EDITORONLY_DATA
	Inspection->bVisualizeComponent = true;
#endif

	MusicManager = CreateDefaultSubobject<UGameMusicManager>("MusicManager");
	MusicManager->SetupAttachment(GetCapsuleComponent());

	Narrative = CreateDefaultSubobject<UToroNarrative>("Narrative");
...
void AGamePlayerChar::SetHidingSpot(AActor* InActor)
{
	...
	{
		MusicManager->RemoveDipRequest(HidingSpot);
	}
	else if (InActor)
	{
		MusicManager->AddDipRequest(InActor);
	}
...
bool AGamePlayerChar::TryJumpscare()
{
	...
	MusicManager->RemoveDipRequest(GetPlayerController());
	MusicManager->RemoveDipRequest(this);
...
FHitResult AGamePlayerChar::OnInteraction() const
{
	...
	{
		PC->GetHitResultUnderCursor(InteractTrace, true, Hit);
	}
	else
	{
		FVector Start, End;
		...
		GetWorld()->LineTraceSingleByChannel(Hit, Start, End, InteractTrace, TRACE_PARAMS);
	}
	return Hit;
}

void AGamePlayerChar::OnInspection(const bool bInspecting)
{
	if (AToroPlayerController* PC = GetPlayerController())
	{
		bInspecting ? PC->AddPauseRequest(Inspection) : PC->RemovePauseRequest(Inspection);
	}
...
{
	if (CAN_INPUT) Narrative->ToggleQuests();
}

void AGamePlayerChar::InputBinding_Interact(const FInputActionValue& InValue)
{
	Interaction->SetInteracting(CAN_INPUT && HasControlFlag(PCF_CanInteract) && InValue.Get<bool>(), this);
}

void AGamePlayerChar::InputBinding_Equipment(const FInputActionValue& InValue)
{
	Inventory->SetEquipmentUsage(CAN_INPUT && InValue.Get<bool>());
}
