...
private:
	...
	bool IsArchive() const { return Item ? Item->ItemType == EInvItemType::Archive : false; }
	...
	{
		if (!IsEnabled()) return;
		Amount = Player->Inventory->AddItem(Item, Amount);
		if (Amount == 0)
		{
			SetEnabled(false);
		}
	...
	virtual bool CanEditChange(const FProperty* InProperty) const override
	{
		...
		{
			return bSuper && Item ? Item->ItemType == EInvItemType::Generic : false;
		}
		return bSuper;
	}
#endif
